    ...
    el.type.forEach(key => {
      switch (key) {
        ...
        case 'json':
          ...
          break;
        case 'endpoint':
          newGroup[el.name] = maxOrMin(endpointValidator, el.max || 0, el.min || 0, el.value || null, el)
          break;
        case 'endpointWithoutPort':
            newGroup[el.name] = maxOrMin(endpointWithoutPortValidator, el.max || 0, el.min || 0, el.value || null, el)
            break;
        case 'notRequired':
...
export function IpValidator(control: AbstractControl): { [key: string]: any } | null {
  ...
  const v = control.value
  if (v === '' || v === null || v.indexOf('localhost')==0) {
    return null
  }
  if (!reg.test(v?.trim())) {
...
// endpoint: a valid FQDN or IP
export function endpointValidator(control: AbstractControl): { [key: string]: any } | null {
  const fqdn_reg = /(?=^.{4,253}$)(^((?!-)[a-z0-9-]{0,62}[a-z0-9]\.)+[a-z]{2,63}$)/gm
  const ip_reg = /((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}/g
  const v = control.value
  if (v === '' || v === null || v.indexOf('localhost')==0) {
    return null
  }
  if (!ip_reg.test(v?.trim())) {
    if (!fqdn_reg.test(v?.trim())) {
      return { message: 'validator.endpoint' }
    }
  }
  return null
}

// endpoint address without port
export function endpointWithoutPortValidator(control: AbstractControl): { [key: string]: any } | null {
  const v = control.value
  if (v === '' || v === null ) {
    return null
  }
  if (v.indexOf(':')>=0) {
    return { message: 'validator.endpintWithoutPort' }
  }
  if (v.indexOf('localhost')===0)  return null
  const fqdn_reg = /(?=^.{4,253}$)(^((?!-)[a-z0-9-]{0,62}[a-z0-9]\.)+[a-z]{2,63}$)/gm
  const ip_reg = /((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}/g
  if (!ip_reg.test(v?.trim())) {
    if (!fqdn_reg.test(v?.trim())) {
