...
func (r *ProjectParticipantRepo) CountJoinedParticipantByProjectUUID(uuid string) (int64, error) {
	var count int64
	err := db.Model(&entity.ProjectParticipant{}).
		Where("project_uuid = ? AND status = ?", uuid, entity.ProjectParticipantStatusJoined).
		Count(&count).Error
	if err != nil {
		return 0, err
	}
...
func (r *ProjectParticipantRepo) DeleteByProjectUUID(projectUUID string) error {
	return db.Unscoped().Where("project_uuid = ?", projectUUID).Delete(&entity.ProjectParticipant{}).Error
}
